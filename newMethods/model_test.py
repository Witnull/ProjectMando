...
class TEST_MODEL1(nn.Module):
    def __init__(self, compressed_global_graph_path, feature_extractor=None, node_feature='han', hidden_size=128, num_layers=2,num_heads=8, use_norm=True, device='cpu'):
        ...
        self.output_mode = output_mode  # New parameter for switching between binary and multilabel
        self.xlstm = xlstm_stack(hidden_size)  # Adding xLSTM
         # Dynamic output
       ...
       # print(f"NTypes:{self.symmetrical_global_graph.ntypes}")
        ...
        self.classify = nn.Linear(self.hidden_size, self.out_size if output_mode == 'binary' else num_classes) 
    ...
    def forward(self):
        ...
        # Integrate xLSTM for enhanced sequence learning
        hiddens = self.xlstm(hiddens.unsqueeze(0)).squeeze(0)
        
        output = self.classify(hiddens)
        if self.output_mode == 'multilabel':
            return torch.sigmoid(output)  # Multilabel output
        else:
            return F.log_softmax(output, dim=-1)  # Binary classification output