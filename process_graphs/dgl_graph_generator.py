...
def print_info():
    ...
    '''
    print("\n\n ###################### INFO ##################### \n\n")
    print(f"{Fore.GREEN}This script generates call graphs from Solidity contracts and analyzes them to find potential vulnerabilities. It uses the DGL library for graph representation and analysis.{Style.RESET_ALL}")
    print(f"\nThe script uses a pre-trained GNN model (HAN) to analyze the call graphs (CG) and identify potential vulnerabilities.")
    print(f"\n - Analyze the call graphs for potential code smells or vulnerabilities.")
    print(f"\n - Generate a vulnerability report for a given contract and bug type.")
    ...
    print(f"\n{Back.YELLOW} This script is seems no need to run. Please check the code before running it.{Style.RESET_ALL}")

    input(f"{Back.BLUE}Press Enter to start generating...{Style.RESET_ALL}")
...
if __name__ == '__main__':
    ...
    extracted_graph = './experiments/ge-sc-data/source_code'
    ##compressed_graph = './data/smartbugs/compressed_graphs/compressed_graphs.gpickle'
    compressed_graph = './experiments/ge-sc-data/compressed_graphs/compress_graphs.gpickle'
